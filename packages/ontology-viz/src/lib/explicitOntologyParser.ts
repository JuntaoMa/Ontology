...
const EXPLICIT_TYPE_IRIS: Record<ExplicitOntologyEntityKind, string> = {
  Class: `${NS.OWL}Class`,
...
const ENTITY_KIND_BY_TYPE_IRI = new Map(
  Object.entries(EXPLICIT_TYPE_IRIS).map(([kind, iri]) => [iri, kind as ExplicitOntologyEntityKind]),
);

interface ParsedStatement {
  subject: string;
  predicate: string;
  object: ExplicitOntologyValue;
}
...
function contentTypeFor(content: string, options: ExplicitOntologyParseOptions) {
  if (options.contentType) return options.contentType;
  return content.trimStart().startsWith("<") ? "application/rdf+xml" : "text/turtle";
}

function valuesByPredicate(statements: ParsedStatement[], subject: string, termType?: ExplicitOntologyValue["termType"]) {
  const grouped: Record<string, ExplicitOntologyValue[]> = {};
...
function firstLiteral(entity: ExplicitOntologyEntity, predicates: string[]) {
...
export function getExplicitOntologyFieldValues(entity: ExplicitOntologyEntity, fieldId: string): string[] {
...
export function getExplicitOntologyDisplayValue(entity: ExplicitOntologyEntity, fieldId: string) {
  return getExplicitOntologyFieldValues(entity, fieldId)[0] ?? "";
}

export function getExplicitOntologyDefaultLabel(entity: ExplicitOntologyEntity) {
  return (
    firstLiteral(entity, [
      iri("RDFS", "label"),
      iri("SKOS", "prefLabel"),
...
export function getExplicitOntologyDefaultDescription(entity: ExplicitOntologyEntity) {
...
function edgeKindLabel(kind: ExplicitOntologyEdgeKind) {
  const labels: Record<ExplicitOntologyEdgeKind, string> = {
...
function buildFields(entities: ExplicitOntologyEntity[]) {
  const fieldMap = new Map<string, ExplicitOntologyField>();

  const addField = (field: ExplicitOntologyField) => {
...
function buildEdges(entities: ExplicitOntologyEntity[]) {
  ...
  const edges: ExplicitOntologyEdge[] = [];
  const seen = new Set<string>();
  const push = (edge: ExplicitOntologyEdge) => {
...
export function parseExplicitOntology(
  content: string,
  options: ExplicitOntologyParseOptions = {},
): ExplicitOntologyGraphData {
  ...
  const entityTypes = new Map<string, ExplicitOntologyEntityKind>();
  ...
  const entities: ExplicitOntologyEntity[] = [...entityTypes.entries()]
    ...
    .map(([subject, kind]) => ({
      ...
      kind,
      typeIRI: EXPLICIT_TYPE_IRIS[kind],
  ...
  const stats: Record<ExplicitOntologyEntityKind, number> = {
    Class: 0,
    ObjectProperty: 0,
    DatatypeProperty: 0,
