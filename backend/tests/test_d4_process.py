...
def test_normal_process_clean(ctx):
    assert ctx.results["v4.formal"].metrics["loan_normal"]["sound"] is True
    assert not _of(ctx, "v4.formal", "loan_normal")
    sim = ctx.results["v4.simulation"].metrics["loan_normal"]
    assert sim["data_coverage_pct"] == 100.0
    assert not _of(ctx, "v4.cross", "loan_normal")


# AC-P-DEADLOCK：XOR-split 配 AND-join 被 soundness 抓
def test_deadlock_caught_by_soundness(ctx):
    assert ctx.results["v4.formal"].metrics["loan_deadlock"]["sound"] is False
    assert _of(ctx, "v4.formal", "loan_deadlock", "process_unsound")


# AC-P-DEADBRANCH：数据感知仿真 0 执行；控制流 play-out 可达（差异演示）
def test_dead_branch_caught_by_data_simulation(ctx):
    assert ctx.results["v4.formal"].metrics["loan_dead_branch"]["sound"] is True  # 结构健全
    sim = ctx.results["v4.simulation"].metrics["loan_dead_branch"]
    assert sim["zero_activities_data"] == ["express_channel"]
    assert sim["data_coverage_pct"] < 100.0
    fs = _of(ctx, "v4.simulation", "loan_dead_branch", "dead_activity_data")
...
def test_rule_violation_caught_by_cross_loop(ctx):
    assert ctx.results["v4.formal"].metrics["loan_rule_violation"]["sound"] is True
    fs = _of(ctx, "v4.cross", "loan_rule_violation", "cross_validation_violation")
    assert len(fs) == 1
    f = fs[0]
...
def test_edge_unfaithful_missed_by_v4(ctx):
    assert ctx.results["v4.formal"].metrics["loan_edge_unfaithful"]["sound"] is True
    assert not _of(ctx, "v4.formal", "loan_edge_unfaithful")
    assert not _of(ctx, "v4.simulation", "loan_edge_unfaithful")
    assert not _of(ctx, "v4.cross", "loan_edge_unfaithful")
