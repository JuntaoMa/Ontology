...
def test_o2_datatype_violation(loan_ctx):
    assert _has(_findings(loan_ctx, "instance.data-quality"), "p002", "shacl_datatype")


def test_o3_enum_violation(loan_ctx):
    assert _has(_findings(loan_ctx, "instance.data-quality"), "app003", "shacl_enum")
...
def test_o5_relation_range_violation(loan_ctx):
    assert _has(_findings(loan_ctx, "instance.data-quality"), "app005", "shacl_class_range")


def test_o6_disjoint_violation_by_reasoner(loan_ctx):
    cons = _findings(loan_ctx, "schema.consistency")
    assert _has(cons, "p007", "disjoint_violation")


def test_o7_functional_violation(loan_ctx):
    assert _has(_findings(loan_ctx, "instance.data-quality"), "p008", "shacl_max_count")


# ---------- AC-O8：pitfalls ----------

def test_o8_pitfalls(loan_ctx):
    pits = _findings(loan_ctx, "schema.pitfalls")
...
def test_o9_semantic_defect_missed_by_deterministic_layers(loan_ctx):
    for vid in ("instance.required-fields", "instance.data-quality", "schema.consistency"):
...
def test_cq_results(loan_ctx):
    cq = _findings(loan_ctx, "instance.competency")
    failed_ids = {f.object_id for f in cq}
    assert failed_ids == {"cq_002", "cq_003"}                 # 数据缺口 + 本体缺口
    assert loan_ctx.results["instance.competency"].metrics["cq_passed"] == 4
...
def test_pizza_seeded_inconsistency_caught():
    ...
    cons = ctx.results["schema.consistency"].findings
    assert any(f.object_id.endswith("weirdBase") for f in cons)
    assert ctx.results["instance.competency"].verdict == "pass"
