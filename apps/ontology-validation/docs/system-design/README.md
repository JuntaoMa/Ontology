...
这是本项目的**目标系统设计基线**。它描述本体制品从入口、编译、并行校验、语义集成、任务验收到
发布签发的完整目标能力，不等同于当前参考 Demo 已实现的功能集合。

当前注册表包含 13 个章节、196 个原子校验项（其中 81 个标记为 core）、11 类作用域，以及由
6 个阶段、14 个模块组成的目标 DAG。当前 DAG 引用 71 个原子校验项；参考 runtime 的 13 个
validator 通过 `../../specs/runtime-design-map.json` 映射到其中 35 个设计项。映射表示工程追踪，
不表示其余设计项已实现。
...
- `../../README.md`：项目入口，明确目标设计与参考 Demo 的边界。
- `../../specs/runtime-design-map.json`：当前 runtime validator 到目标设计项的职责映射。
- `../overview.md`：已实现 loan Demo 的导览。
- `../design-plan.md` 与 `../../specs/00-master-spec.md`：V0–V6 参考实现的历史设计和验收记录。

设计注册表是目标能力目录和编排蓝图，不应根据参考 Demo 的现状删减。反过来，README、API 或测试中
出现某个 runtime validator，也不意味着与之映射的设计项已经具备完整的独立执行契约。
