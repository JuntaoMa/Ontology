...
2. 对 `expanded_mpart_tree` 中按 `MBOMNode.depth` 从大到小排序的每个 `node` 执行
   ...
   - 调用 `check_inventory(required_qty, node_inventory_position, node_inventory_lots)` 作为 `inventory_coverage`
   - 如果 `node.MPart.partKind = "leaf"`
     则
     - 调用 `calc_leaf_ready(node.MPart, inventory_coverage, node_purchase_orders, node_certifications, Context.now)` 作为 `part_ready_cycle`
   - 否则
     - 设 `child_cycles` = 从 `result_list` 中选取 `parentMbomNodeId = node.MBOMNode.id` 的下层结果
     - 调用 `calc_assembly_ready(node.MPart, inventory_coverage, child_cycles, Context.now)` 作为 `part_ready_cycle`
