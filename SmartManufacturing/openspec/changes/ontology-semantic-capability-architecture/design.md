...
### 7. 前端 demo 采用四页式产品结构，而不是单页三栏工作台

前端 demo 将采用四个主要页面承载四类演示能力：
...
### 8. About 页作为用户导向的总说明入口

`About` 页面面向业务用户和演示观众，负责用准确但非实现细节导向的语言解释：

- 本体平台是什么
- 什么是语义层
- 什么是能力层
- Agent 如何围绕本体处理业务问题
- 一个典型问题会如何映射到对象、关系和能力

该页面不承担编辑功能，也不展开底层 schema 细节。它的职责是帮助用户在进入图谱页和问答页之前，先建立统一的概念模型。

### 9. 本体查看与管理页作为前端 demo 的核心页面
...
页面组织调整为“图谱主画布 + 次级抽屉/窗口”：

- 顶部：domain 选择、搜索、图谱视角切换
- 中央：可拖拽的异构图主画布，突出 `Object / Link / Function / Action` 之间的连接结构
- 右侧或浮层：当前选中资源摘要
- 抽屉 / 模态窗口：元数据编辑、能力边界、结构化逻辑体和依赖详情

这里不再以左侧资源树为主，而是让图谱成为默认入口。本体管理页最重要的不是资源表格，而是“结构如何组织、能力如何挂接到对象上”。

### 10. Agent 问答页采用“左对话、右上下文与推理链”的双栏结构
...
### 12. 前端实现采用 React + TypeScript 多页面应用
...
- 需要比静态页面更稳定的状态组织和交互能力
- 需要在多页面之间共享当前 domain、最近一次问答结果和资源定位信息
- 需要支持图谱页面的复杂交互、抽屉编辑和问答页的持续状态展示

这一决策替代此前“零依赖静态 demo”实现。

### 12. 本体图谱使用 React Flow 作为交互式图结构组件

本体查看页的图谱组件选型为 `React Flow`。它需要满足：

- 节点可拖拽
- 节点可点击打开详情
- 支持缩放、平移、小地图和视图重置
- 能在一个画布中同时承载语义边与能力依赖边

在当前阶段，节点和边不需要支持复杂在线建模，只需支持：
...
选择 `React Flow` 的原因是它对异构节点、交互控制和 React 集成更直接，适合作为数据平台风格 demo 的主画布。
