...
- **WHEN** agent 读取 `manufacture-design-change/ontology/actions.json`
- **THEN** 可以看到 action 的输入输出、依赖查询工具、依赖函数和 `logicRef`
- **AND** 需要读取 `logic/actions/*.md` 才能看到完整编排

### Requirement: manufacture-design-change 必须提供已知业务 action
系统 MUST 为 `manufacture-design-change` 提供一个已落地 action：`create_report`。

#### Scenario: 处理第三批次物料可用周期问题
- **WHEN** agent 处理第三批次切换的新物料可用周期问题
- **THEN** 可以直接使用 `create_report` 组织完整流程
...
- **WHEN** agent 读取 `create_report` 的逻辑体
...
- **THEN** 系统可以先复用现有查询能力，再用新增 `calc_scrap_cost(...)` 草案和临时编排输出业务报告
